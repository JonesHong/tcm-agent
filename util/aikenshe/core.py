...
from .api import api_router, quanxi_analysis_local_file
from .socket import emit_message, sio
...
def handle_vip_event(data_parsed): 
    ...
    print(f'is_photo_taken: {is_photo_taken}')
    # pass
...
async def process_local_file(age, male, file_path):
    ...
    if payload is not None:
        ...
        # 發佈消息到 Redis 頻道
        redis_core.publisher(RedisChannel.aikanshe_done_service, result_string)
    else:
        # 發佈消息到 Redis 頻道
        redis_core.publisher(RedisChannel.aikanshe_done_service, '')
...
redis_core = RedisCore(config=config.redis,channel=channels,message_handler=message_handler)
