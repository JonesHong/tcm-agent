...
from util.agent.rag_chain import JiudaTizhi, ZhongyiShiwen, llm
from util.redis.channel import RedisChannel
from util.redis.core import RedisCore
from abc import ABC, abstractmethod

config = ConfigManager()
redis_core = RedisCore(config.redis)
...
class DiagnosticStrategy(InteractionStrategy):
    def handle_interaction(self, agent, user_input):
        ...
        # elif (agent.sex == 'male' and agent.question_count >= 9) or (agent.sex == 'female' and agent.question_count >= 10):
            ...
            zhongyi_shiwen_response = agent.interrogation_details[key]
            # print(agent.interrogation_details,key,zhongyi_shiwen_response)
            
            # 儲存問診問題
            question_key = agent.qa[agent.question_count]
            # agent.questions[question_key] = zhongyi_shiwen_response['answer']
            agent.questions[question_key] = zhongyi_shiwen_response
            # print(agent.questions[question_key])
...
class Silent(InteractionStrategy):
    def handle_interaction(self, agent, user_input):
        # 沉默模式的具體邏輯在這裡實現
        print("現行版本，流程上僅提供十問和舌診，評估與建議以後就保持沉默，等使用者離開")
        # response = llm.invoke(f"Chitchat response for input: {user_input}")
        # agent.messages.append(response)
        # return response
        pass