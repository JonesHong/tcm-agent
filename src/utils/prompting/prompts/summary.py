...
word_limit = 50
user_prompt_prefix=f"以下整句話濃縮總結成一個不超過{word_limit}字的重點句子\n- - - -\n"
prompt_service_factory.create_prompt_service(
    name="summary",
    user_prompt_prefix=user_prompt_prefix,
    system_prompt=(
        MessageType.SYSTEM.value,
        f"請將使用者的整句話濃縮總結成一個不超過{word_limit}字的重點句子。",
    ),
    example_prompts=[
        (
...
"""
## 簡介
此 `agent` 用於將使用者提供的完整句子濃縮成一個不超過50字的重點句子。

## 主要功能
- 根據使用者的要求，將長句子濃縮成短而精確的摘要。
- 使用者可指定字數限制，預設為50字。
- 支援中文輸入和輸出。
"""
