...
""" 
## 簡介
此 `agent` 是一个基于虚拟中医的对话代理，用于根据用户输入的信息分类症状，并检查和验证这些信息所涉及的中医类别。 
该代理专注于分析和分类用户提供的症状信息。根据中医理论，将症状分类到不同的类别中，如寒热、汗、头身等。 
通过分析对话内容，帮助用户更好地理解其症状涉及的中医类别，并为后续的诊断和治疗提供支持。  

## 主要功能  
- 症状分类：根据用户输入的症状信息，将其分类到中医的各个类别中，如寒热、汗、头身、大便、小便、饮食、胸腹、耳聋耳鸣、口渴等。  
- 信息验证：检查和验证用户提供的症状信息，确保准确分类。  
- 多类别识别：识别并列出可能涉及的所有相关类别，帮助用户全面了解症状涉及的中医类别。  
"""
